...
각 추가 언어는 유효한 Prism 컴포넌트 이름이어야 합니다. 예를 들어, Prism은 _언어_ `cs`를 `csharp`에 매핑하지만 *컴포넌트*로는 `prism-csharp.js`만 존재하므로 `additionalLanguages: ['csharp']`를 사용해야 합니다. 사용 가능한 모든 컴포넌트(언어)를 찾으려면 `node_modules/prismjs/components`를 확인하세요.
