...
[사이트 구성](./configuration.mdx#site-metadata)을 통해 전체 사이트에 대한 전역 메타 속성을 제공합니다. 메타데이터는 모두 HTML `<head>`에 키-값 쌍을 속성 이름과 값으로 사용하여 렌더링됩니다. `metadata` 속성은 `<meta>` 태그를 선언하는 편리한 단축키이지만, `headTags` 속성을 사용하여 `<head>`에 임의의 태그를 주입하는 것도 가능합니다.
...
Docusaurus는 각 Markdown 페이지에 `description`, `title`, 정규 URL 링크 및 기타 유용한 메타데이터를 자동으로 추가합니다. 이들은 [프론트 매터](./guides/markdown-features/markdown-features-intro.mdx#front-matter)를 통해 구성할 수 있습니다:
...
`description`과 `keywords`와 같은 필드에는 [프론트 매터](./guides/markdown-features/markdown-features-intro.mdx#front-matter)를 사용하는 것이 좋습니다: Docusaurus는 이를 자동으로 `description`과 `og:description` 모두에 적용하지만, `<head>` 태그를 사용할 때는 두 메타데이터 태그를 수동으로 선언해야 합니다.

:::

:::info

공식 플러그인은 모두 다음과 같은 [프론트 매터](./guides/markdown-features/markdown-features-intro.mdx#front-matter)를 지원합니다: `title`, `description`, `keywords`, `image`. 추가적인 [프론트 매터](./guides/markdown-features/markdown-features-intro.mdx#front-matter) 지원은 각 플러그인의 API 문서를 참조하세요:

- [Docs 프론트 매터](./api/plugins/plugin-content-docs.mdx#markdown-front-matter)
- [Blog 프론트 매터](./api/plugins/plugin-content-blog.mdx#markdown-front-matter)
- [Pages 프론트 매터](./api/plugins/plugin-content-pages.mdx#markdown-front-matter)
...
편의를 위해 기본 테마 `<Layout>` 컴포넌트는 `title`과 `description`을 속성으로 받습니다.
...
대부분의 [CommonMark](https://spec.commonmark.org/0.30/#atx-headings) 문법은 해당하는 HTML 태그를 가지고 있습니다. 프로젝트에서 Markdown을 일관되게 사용하면 검색 엔진이 페이지 콘텐츠를 더 쉽게 이해할 수 있습니다.
