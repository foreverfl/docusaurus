---
description: Docusaurus는 웹사이트를 생성, 제공 및 배포하는 데 도움이 되는 스크립트 세트를 제공합니다.
---

# CLI

Docusaurus는 웹사이트를 생성, 제공 및 배포하는 데 도움이 되는 스크립트를 제공합니다.

웹사이트가 초기 설정을 완료하면, 웹사이트 소스에는 패키지 관리자로 호출할 수 있는 Docusaurus 스크립트가 포함됩니다:
...
[Webpack Dev Server](https://webpack.kr/configuration/dev-server/)를 사용하여 사이트의 미리보기를 로컬에서 빌드하고 제공합니다.
...
| `--hot-only` | `false` | 빌드 실패 시 페이지 새로 고침 없이 핫 모듈 교체를 활성화합니다. 자세한 정보는 [여기](https://webpack.kr/configuration/dev-server/#devserverhotonly)에서 확인할 수 있습니다. |
| `--no-open` | `false` | 브라우저에서 페이지를 자동으로 열지 않습니다. |
| `--config` | `undefined` | Docusaurus 설정 파일 경로, 기본값은 `[siteDir]/docusaurus.config.js`입니다. |
| `--poll [optionalIntervalMs]` | `false` | 감시 기능이 제대로 작동하지 않는 환경(예: 네트워크 드라이브, 가상화 환경)에서는 폴링(polling)을 사용하여 라이브 리로드를 작동시킬 수 있습니다. 자세한 정보는 [여기](https://webpack.kr/configuration/watch/#watchoptionspoll)에서 확인할 수 있습니다. |
...
| `--eject` | 테마 컴포넌트를 [분리](./swizzling.mdx#ejecting)합니다 |
| `--wrap` | 테마 컴포넌트를 [감싸써](./swizzling.mdx#wrapping) 확장합니다 |
| `--danger` | 안전하지 않은 컴포넌트의 즉시 swizzle을 허용합니다 |
| `--typescript` | TypeScript 변형 컴포넌트를 swizzle합니다 |
| `--config` | docusaurus 설정 파일 경로, 기본값은 `[siteDir]/docusaurus.config.js`입니다. |
...
| 이름 | 기본값 | 설명 |
| --- | --- | --- |
| `--port` | `3000` | 지정된 포트를 사용합니다. |
| `--dir` | `build` | 현재 작업 공간을 기준으로 한 출력 디렉토리의 전체 경로입니다. |
| `--build` | `false` | 제공하기 전에 웹사이트 빌드합니다. |
| `--config` | `undefined` | Docusaurus 설정 파일 경로, 기본값은 `[siteDir]/docusaurus.config.js`입니다. |
| `--host` | `localhost` | 사용할 호스트를 지정합니다. 예를 들어, 서버를 외부에서 접근 가능하게 하려면 `--host 0.0.0.0`을 사용할 수 있습니다. |
