...
기본적으로 `_`로 시작하는 Markdown 또는 JavaScript 파일은 무시되며 해당 파일에 대한 라우트가 생성되지 않습니다. `exclude` 옵션을 참조하세요.
...
실수로 동일한 라우트에 접근하도록 의도된 여러 페이지를 만들 수 있습니다. 이런 일이 발생하면 Docusaurus는 `yarn start` 또는 `yarn build`를 실행할 때 중복 라우트에 대해 경고합니다. 이 동작은 [`onDuplicateRoutes`](../api/docusaurus.config.js.mdx#onDuplicateRoutes) 설정을 통해 구성 가능하며, 사이트는 성공적으로 빌드됩니다. 마지막에 생성된 페이지가 접근 가능하지만, 다른 충돌하는 페이지를 덮어씁니다. 근본적으로 이 문제를 해결하려면 충돌하는 라우트를 수정하거나 제거해야 합니다.
